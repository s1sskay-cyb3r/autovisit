...
$hijau = "\e[92m";
$merah = "\e[91m";
echo "$cyan [+]=============[Auto Visitor Blog By hadsyhputra]=============[+]\n";
echo "$biru /*  Auto Visitor Blog v.0.1            */\n";
echo "$cyan /* Created Date : 28-02-2019           */\n";
echo "$magenta /* Contact Me: hadsyh@outlook.com      */\n";
echo "$kuning /* Thanks to : PacmanCorp Team         */\n";
echo "$cyan [+]=============[Pake Jgn Berlebihan Klo ga mau error]=========[+]\n";
echo "$hijau [1] Link Postingan Blog : "; 
$url = trim(fgets(STDIN));
echo "$cyan [2] Masukan jumlah : "; 
...
for($i = 1; $i < $max+1; $i++) {
	...
	echo $i.". Status => 200 OK ~ [".$class->jalankan()."]\n";
}
